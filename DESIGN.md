...
- **Multi-Pane Diff/Merge**: Four or more adjacent panes in a custom layout.

### Multi-Column Implementation
While 2 and 3-way views utilize the native CodeMirror Merge addon, views with four or more panes use a custom implementation:
- **Synchronization**: Scroll events in any pane are mirrored to all others.
- **Diff Calculation**: Uses `diff_match_patch` to calculate line-level chunks between the first file and each other file.
- **Mapping**: Boundary mapping translates line ranges between panes using calculated diff chunks, ensuring that line moves target the corresponding range even if files have diverged in length.
